...
int kernelPayload(struct thread *td, void* uap) {
    ...
    *(uint8_t *)(ptrKernel + 0x2DDDFD) = 0xEB;//11
    ...
    // patch sceSblACMgrIsAllowedSystemLevelDebugging
    memcpy((void *)(ptrKernel + 0x3D0DE0), "\x48\xC7\xC0\x01\x00\x00\x00\xC3", 8);

    // patch sceSblACMgrHasMmapSelfCapability
    memcpy((void *)(ptrKernel + 0x3D0E50), "\x48\xC7\xC0\x01\x00\x00\x00\xC3", 8);

    // patch sceSblACMgrIsAllowedToMmapSelf
    memcpy((void *)(ptrKernel + 0x3D0E70), "\x48\xC7\xC0\x01\x00\x00\x00\xC3", 8);

    // disable sysdump_perform_dump_on_fatal_trap
    // will continue execution and give more information on crash, such as rip
    *(uint8_t *)(ptrKernel + 0x76D210) = 0xC3;

    // self patches
    memcpy((void *)(ptrKernel + 0x157F91), "\x31\xC0\x90\x90\x90", 5);

	// patch ptrace, thanks 2much4u
	*(uint8_t*)(ptrKernel + 0x384285) = 0xEB;
	memcpy((void*)(ptrKernel + 0x384771), "\xE9\xD0\x00\x00\x00", 5);

    // Disable process aslr
    *(uint16_t*)&ptrKernel[KERN_PROCESS_ASLR] = 0x9090;//9

    // patch kmem_alloc
    *(uint8_t *)(ptrKernel + 0x245EDC) = VM_PROT_ALL;
    *(uint8_t *)(ptrKernel + 0x245EE4) = VM_PROT_ALL;
